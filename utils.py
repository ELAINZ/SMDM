...
def _resolve_ckpt_path(model_name: str):
    """Resolve checkpoint path: local file or download from HuggingFace (repo_id/filename.pth)."""
    path = Path(model_name)
    if path.suffix.lower() == ".pth" and path.exists():
        return str(path.resolve())
    # HuggingFace: "org/repo/file.pth"
    if "/" in model_name and model_name.lower().endswith(".pth"):
        try:
            ...
            raise FileNotFoundError(
                f"Cannot resolve checkpoint: {model_name} (local file not found, HF download failed: {e})"
            )
    return model_name
...
def load_model_and_tokenizer(
    ...
    device: str = None,
    local_rank: int = None,
    diff_config_name: str = None,
):
    """
    Load model and tokenizer with appropriate settings based on model type.
    
    Args:
        model_name: Name or path of the model (HuggingFace id, local path, or .pth for diffusion)
        ...
        diff_config_name: For .pth diffusion checkpoints, config name (e.g. "Diff_LLaMA_551M").
                         Default "Diff_LLaMA_551M" when loading .pth / smdm / cdlm.
    ...
    # Check if this is the diffusion/TransEncoder model (evaluate_gsm8k-style)
    is_smdm = (
        model_name.lower().endswith(".pth")
        or "smdm" in model_name.lower()
        or "cdlm" in model_name.lower()
        or "diff_llama" in model_name.lower()
    )
    
    if is_smdm:
        # Same loading as evaluate_gsm8k.py: Config + TransEncoder + TinyLlama tokenizer
        from lit_gpt.model_cache import Config
        from lit_gpt.diffmodel import TransEncoder
        
        ckpt_path = _resolve_ckpt_path(model_name)
        config_name = diff_config_name or "Diff_LLaMA_551M"
        print(f"[Rank {local_rank}] Loading diffusion model: {config_name} from {ckpt_path}")
        
        config = Config.from_name(config_name)
        model = TransEncoder(config)
        ckpt = torch.load(ckpt_path, map_location="cpu")
        model.load_state_dict(ckpt["model"])
        # bfloat16 required: TransEncoder.build_rope_cache uses dtype=bfloat16; activations must match
        ...
        pad_id = 32000
        mask_id = 32000  # TransEncoder uses padded_vocab_size as mask token (same as diff_sample)
        print(f"[Rank {local_rank}] Loaded TransEncoder (evaluate_gsm8k-style), mask_id={mask_id}")
