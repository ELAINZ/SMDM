...
def _is_diffusion_transencoder(model):
    """True if model is lit_gpt.diffmodel.TransEncoder (e.g. from evaluate_gsm8k.py).
    For this model, use mask_id=32000 (padded_vocab_size) when calling llada_sample."""
    cls = type(model)
    return cls.__name__ == "TransEncoder" and "lit_gpt" in getattr(cls, "__module__", "")
...
def get_model_logits(model, input_ids, attention_mask=None, is_causal=None):
    ...
    """
    # TransEncoder (evaluate_gsm8k / diff_sample style) only accepts input_ids
    if _is_diffusion_transencoder(model):
    ...
    # 1. Standard transformers models: output.logits
    # 2. TransEncoder (SMDM diffusion): output may be BaseModelOutputWithPastAndCrossAttentions
    #    or directly logits tensor
    # 3. Encoder-style models: output.last_hidden_state only -> get logits via model.lm_head
    if hasattr(output, 'logits'):
        return output.logits
    elif isinstance(output, torch.Tensor):
        # Direct tensor output (e.g., TransEncoder)
        return output
    elif hasattr(output, 'last_hidden_state'):
        # Model returned hidden states only; get logits via lm_head if available
        hidden = output.last_hidden_state
        if hasattr(model, 'lm_head'):
            return model.lm_head(hidden)
        if hasattr(model, 'head'):
            return model.head(hidden)
        # Try common nested names (e.g. transformer.lm_head, backbone.lm_head)
        for sub in ('transformer', 'backbone', 'encoder', 'model'):
            subm = getattr(model, sub, None)
            if subm is not None and hasattr(subm, 'lm_head'):
                return subm.lm_head(hidden)
            if subm is not None and hasattr(subm, 'head'):
                return subm.head(hidden)
        for attr in ('output_projection',):
            obj = getattr(model, attr, None)
            if obj is not None and callable(obj):
                return obj(hidden)
        # TransEncoder (lit_gpt.diffmodel): forward() returns logits when called with
        # only input_ids; passing attention_mask can cause HF-style last_hidden_state.
        # Retry with no kwargs to get logits directly (same as reference: model(x)).
        if kwargs:
            output_no_kw = model(input_ids)
            if isinstance(output_no_kw, torch.Tensor):
                return output_no_kw
        raise ValueError(
            "Model output has last_hidden_state but no logits and no lm_head/head on the model. "
            "TransEncoder or encoder-style models should either return logits or expose lm_head."
        )
    else:
        # Try to get logits from tuple if output is a tuple
        if isinstance(output, tuple) and len(output) > 0:
            return output[0]
        raise ValueError(f"Unable to extract logits from model output. Output type: {type(output)}")
...
def llada_sample(
    ...
    mad_k: float = 2.5,
    remask_only_variable_indices: Optional[list] = None,
) -> Dict:
    """
    Generate text using LLaDA with remask-based sampling.

    Args:
        model: LLaDA model (or TransEncoder from evaluate_gsm8k / lit_gpt.diffmodel;
               for TransEncoder use mask_id=32000)
        input_ids: [batch_size, seq_len] token IDs (with masks already)
        fix_mask: [batch_size, seq_len] True=fixed, False=variable
        mask_id: mask token ID (use 32000 for TransEncoder / diffusion model)
        ...
        remask_only_variable_indices: optional list of length batch_size; each
               element is a list of 0-based variable indices that are allowed
               to be remasked. Empty list for a sample means allow all variable
               positions. None means no restriction.
    ...
    # Build optional mask: only allow remasking at these variable indices (e.g. stage-2 number-only)
    remask_allow_mask = None
    if remask_only_variable_indices is not None and len(remask_only_variable_indices) == batch_size:
        remask_allow_mask = torch.ones_like(x, dtype=torch.bool)
        for i in range(batch_size):
            allowed = set(remask_only_variable_indices[i])
            if not allowed:
                continue  # empty = allow all variable positions for this sample
            var_idx = 0
            for j in range(seq_len):
                if not fix_mask[i, j]:
                    if var_idx not in allowed:
                        remask_allow_mask[i, j] = False
                    var_idx += 1
    ...
    for step in range(steps):
        ...
        if remask_allow_mask is not None:
            to_remask = to_remask & remask_allow_mask
